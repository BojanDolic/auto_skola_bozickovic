package com.autoskola.bozickovic

object Constants {

    const val KATEGORIJA_A = "A"
    const val KATEGORIJA_B = "B"
    const val KATEGORIJA_C = "C"

    const val CONSTANT_TEORIJA = "teorija"
    const val CONSTANT_ZNAK = "znak"
    const val CONSTANT_RASKRSNICA = "raskrsnica"
    const val CONSTANT_PROBNI = "sve" // Znači da je izabran probni test (bira sve iz baze)

    const val KATEGORIJA_C_BODOVA = 140
    const val KATEGORIJA_AB_BODOVA = 120

    const val KATEGORIJA_C_MIN_BODOVA = 126
    const val KATEGORIJA_AB_MIN_BODOVA = 108

}